...
import { getAppSecretsCacheSetings, getCacheSettings, getConnectorSettings, getExperimentsCacheSettings, getGeoLookupCacheSettings, getPlatformSettingsSync } from '../utils/platform-utils';
...
const createAndGetConnectorCache = async () => {
...
const getGeoLookupConnector = async (connectorSettings) => {
    if (connectorSettings && connectorSettings.geoLookup) {
        ...
        msdyn365Commerce.getConnector(connectorSettingsForGeoLookup.name)?.provider);
        ...
        try {
            ...
            if (!geoLookupProviderReady) {
                console.error(`geoLookup connector registered with name ${connectorSettingsForGeoLookup.name} not initialized successfully`);
                appInsights.defaultClient.trackTrace({
...
const getExperimentaionConnector = async (secretManager, connectorSettings) => {
