...
export const constructViewportInformationForImage = (moduleName, moduleLayout, pathToField, quality, themeSettings, focalRegion = {}, cropRegions = [], format) => {
    ...
    Object.keys(imageSettings.viewports).forEach(key => {
        if (imageSettings.viewports[key].height) {
            imageSettings.viewports[key].h = imageSettings.viewports[key].h || imageSettings.viewports[key].height;
            delete imageSettings.viewports[key].height;
        }
        if (imageSettings.viewports[key].width) {
            imageSettings.viewports[key].w = imageSettings.viewports[key].w || imageSettings.viewports[key].width;
            delete imageSettings.viewports[key].width;
        }
    });
...
const constructImageQueryString = (width, height, quality, focalRegion, cropRegions, moduleName, moduleLayout, pathToField, viewport, format) => {
    ...
    Object.keys(resizerProperties).forEach((key) => {
        if (resizerProperties[key] && resizerProperties[key] !== '') {
