    ...
    _putRequestedModulesInCacheOnServer(pageData, resourceManager) {
        ...
        for (let i = 0; i < pageData.slots.length; ++i) {
            ...
            if (resourceManager && !isEditor) {
                this.requestCache.put({ typeName: MODULE_RESOURCES_KEY, key: moduleInstance.typeName }, { item: resourceManager.getModuleResources(moduleInstance.typeName, this.requestContext.locale) });
            }
        }
        ...
        if (isEditor && resourceManager && this.requestContext.features.enableAuthoringRemoveAndAddModule) {
            ...
            bindings.forEach((moduleBinder) => {
                this.requestCache.put({ typeName: MODULE_RESOURCES_KEY, key: moduleBinder.name }, { item: resourceManager.getModuleResources(moduleBinder.name, this.requestContext.locale) });
            });
        }
    }
    /**
