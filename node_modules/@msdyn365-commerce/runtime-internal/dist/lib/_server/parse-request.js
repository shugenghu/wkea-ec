...
export const getApiSettingsFromEnv = () => {
    return {
...
export const getDefaultRequestContext = (req, apiSettingsFromEnv, concatJs) => {
    return {
        url: {
        ...
        params: {
            ...
            concatJs: concatJs && concatJs.hasValue
                ? concatJs
                : {
                    hasValue: true,
                    isTruthy: false,
...
export const parseRequest = (req) => {
    const apiSettingsFromEnv = getApiSettingsFromEnv();
    const concatJs = parseToQSPObject(req.query.concatJs);
    const serverProperties = Object.create({
        headers: getHeaders(req)
    });
    return Object.assign(serverProperties, getDefaultRequestContext(req, apiSettingsFromEnv, concatJs));
};
...
export const mapRequestContextWithRenderingContext = (req, requestContext, renderingContext, appContext = getDefaultAppSettings(), pageConfig = getDefaultPageConfig()) => {
    ...
    const cartCookieExpiration = (appContext && appContext.platform && appContext.platform.cartSessionExpiration) || 0;
...
const addConnectorInformation = () => {
    ...
    if (msdyn365Commerce.geoLookupConnector) {
        connectors.geoLookupConnector = msdyn365Commerce.geoLookupConnector.name;
    }
    return connectors;
};
