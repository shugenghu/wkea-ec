...
let SafeRenderModule = class SafeRenderModule extends React.Component {
    constructor(props) {
        ...
        this.props.internalTelemetry.log(LogLevel.Debug, `Safe Render module constructor type-'{typeName}' with id-'{id} and is ${this.disableDefer ? 'not ' : ''}using defer module.'`, {
...
export const getRenderSlotComponent = (parentProps, moduleConfigs, slotId, addSiblingIndices) => {
    if (!moduleConfigs) {
        return [];
    }
    ...
    return moduleConfigs.map((moduleConfig, index) => {
        return addSiblingIndices ? (React.createElement(AddModuleWrapper, { safeRenderProps: {
                key: moduleConfig.id,
                ...moduleConfig,
...
const getSlotDictionary = (props, slotsForAddModule) => {
    ...
    if (modules) {
        Object.keys(modules).forEach((key) => {
            slotDictionary[key] = getRenderSlotComponent(props, getNonPlaceholderModules(modules[key]), key, slotsForAddModule ? addSiblingIndexDictionary[key] || [] : undefined);
        });
    }
