...
import { ApiVersion, EntityIdentifier } from './Entities/CommerceTypes.g';
import { AsyncResult, isAsyncResult } from './async-result';
import { ICacheSource } from './Interfaces';
...
import { ODataHttpRequest } from './externals/DataJS.New';
import { LogLevel } from './interfaces/ITelemetry';
...
export const GET_FROM_RETURNIDENTIFIER = '___GET__FROM__RETURNIDENTIIER___';
 ...
 */
export function parseOdataResult(data, resultType) {
    if (!data) {
        return data;
    }
    if (!data.value) {
...
export class DataServiceRequest {
    constructor(query) {
        this._cacheKeyFromReturnType = [];
        this.getActionId = () => this._actionId;
        this.getCacheObjectType = () => this._cacheObjectType;
        this.getCacheKey = () => this._cacheKeyFromInput;
        this.dataCacheType = () => DataServiceRequest.getDataCacheType(this._query);
        this.shouldCacheOutput = () => true;
        this.overrideCacheType = (bypassCache) => {
     ...
     */
    queryParams() {
        return this._query.data.parameters;
    }
    static buildBatchRequest(batchRequestUri, requests) {
        let requestId = 1;
     ...
     */
    static parseBatchResponses(batchResponses, requests, responses) {
     ...
     */
    static toJson(data, parent, modifyDataInPlace) {
        let clone = data;
        if (data === null) {
            return null;
        }
     ...
     * @param {boolean} [modifyDataInPlace] Whether the object passed should be modified in place. If not, a clone is created instead.
     */
    static arrayToJson(array, parent, modifyDataInPlace) {
        const data = [];
     ...
     */
    static formatFunctionParameter(parameters) {
        let result = EMPTY;
        if (parameters) {
    ...
    static addParameter(requestUri, key, value) {
        if (value) {
    ...
    static getActionIdFromInput(query) {
        return `@msdyn365-commerce/retail-proxy/${query.entitySet}/${query.action}`;
    }
    static getCacheObjectValueFromReturnType(returnType) {
    ...
    static buildCacheFromInput(query, cacheKeyIdentifierFromReturnType) {
        ...
        query.data && query.data.parameters && (cacheKey = { ...cacheKey, ...query.data.parameters });
        ...
        const cacheKeyValue = convertToString(cacheKey) ||
    ...
    saveCacheEntries(data, requestCache, appCache, isCacheTypeApplication) {
    ...
    _saveEntityToCache(data, requestCache, appCache, isCacheTypeApplication) {
        if (!data || (!this._cacheKeyFromInput && !this._cacheKeyIdentifierFromReturnType)) {
            return;
        }
        const cacheEntries = {};
    ...
    readCacheEntries(requestCache, appCache, shouldReadFromAppCache, telemetry) {
        if (!requestCache || (!this._cacheKeyFromInput && this._cacheKeyFromReturnType.length === 0)) {
            return;
        }
        if (this._cacheKeyFromInput === GET_FROM_RETURNIDENTIFIER ||
        ...
        return this._resolveCacheEntryReference(cachedValueBasedOnInput.item, requestCache, appCache, shouldReadFromAppCache, isAsyncResult(cachedValueBasedOnInput.item));
    }
    _resolveCacheEntryReference(item, requestCache, appCache, shouldReadFromAppCache, isWrappingPromise) {
    ...
    readCacheEntriesBasedOnCacheKeyFromReturnType(requestCache, appCache, shouldReadFromAppCache, telemetry) {
     ...
     */
    execute(callerContext) {
        if (!callerContext.requestContext ||
        ...
        let result;
        const headers = this.buildQueryHeader();
     ...
     */
    executeBatch(requests, callerContext) {
        if (!callerContext.requestContext ||
            !callerContext.requestContext.apiSettings ||
            !callerContext.requestContext.apiSettings.oun ||
            !callerContext.requestContext.apiSettings.baseUrl) {
        ...
        return new Promise((resolve, reject) => {
            ODataHttpRequest(DataServiceRequest.buildBatchRequest(this.batchRequestUri, requests), (data) => {
                const responses = new Array();
                const errors = DataServiceRequest.parseBatchResponses(data.__batchResponses, requests, responses);
                if (errors.length === 0) {
     ...
     */
    getRequestUri() {
        let requestUri = this._callerContext.requestContext.apiSettings.baseUrl + 'Commerce';
        ...
        if (this._query.isReturnTypeACollection) {
            requestUri = DataServiceRequest.addParameter(requestUri, '$top', this._query.resultSettings && this._query.resultSettings.Paging
                ? this._query.resultSettings.Paging.Top
                : ApiTimeoutInMilliSecond); // Config.defaultPageSize);
            requestUri = DataServiceRequest.addParameter(requestUri, '$skip', this._query.resultSettings && this._query.resultSettings.Paging ? this._query.resultSettings.Paging.Skip : 0);
            ...
            requestUri = DataServiceRequest.addParameter(requestUri, '$filter', this._query.filterSettings ? this._query.filterSettings : 0);
            if (this._query.resultSettings && this._query.resultSettings.Sorting && this._query.resultSettings.Sorting.Columns) {
                this._query.resultSettings.Sorting.Columns.forEach((column) => {
                    if (!isNullOrWhitespace(column.ColumnName || EMPTY)) {
                        const orderByColumn = format('{0}%20{1}', column.ColumnName, column.IsDescending ? 'desc' : 'asc');
     ...
     */
    getAllPages(headers, skip, results) {
        skip = skip || 0;
    ...
    buildQueryHeader() {
        ...
        for (const tokenName in this._query.tokens) {
            if (this._query.tokens.hasOwnProperty(tokenName)) {
                const tokenValue = this._query.tokens[tokenName];
    ...
    loadPage(headers, skip) {
        ...
        return new Promise((resolve, reject) => {
            ...
            ODataHttpRequest({
                ...
                if (this._cacheKeyIdentifierFromReturnType) {
                    if (Object.prototype.toString.call(resultSet) === '[object Array]') {
                        this._cacheKeyFromReturnType.concat(resultSet.map(result => result[this._cacheKeyIdentifierFromReturnType]).filter(Boolean));
                    }
...
//# sourceMappingURL=DataServiceRequest.js.map