...
import { asSystemMetadata, GenericConsoleLogger, LogLevel, StaticTelemetry, Telemetry } from '@msdyn365-commerce/telemetry-internal';
...
export class ActionContext {
    ...
    constructor(args) {
        ...
        this.isMaxDepthExceeded = () => {
            const maxDepth = (this.requestContext &&
            ...
            const outputs = inputsArray.map(i => {
                if (i instanceof DataServiceRequest) {
                    try {
                        ...
                        const entry = readCacheEntries(i, this.requestCache, appCacheData, false, StaticTelemetry);
                        return entry.entry;
                    }
                    catch (e) {
        ...
        return Promise.all(promises)
            ...
            if (refreshconfigslocal.size > 0 && requestContext.features && !!requestContext.features.disable_appcache_refresh_await) {
                refreshconfigslocal.forEach(rf => {
                    ...
                    rf.paramExecutor
                        .execute(refreshContext, refreshRequestCache, this.appCache, this.requestContext.query && !!this.requestContext.query.cachebypass, rf.actionInputs)
