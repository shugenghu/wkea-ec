...
const yargsCLI = yargs
    ...
    .command('pack [packageName] [--skip-health-check] [--use-max-memory-heap] [--skip-validation]', 'Packages partner application. PackageName optional\n', (params) => {
    return params
        .positional('packageName', {
        describe: 'Name of the package'
    })
        .positional('--skip-health-check', {
        describe: 'Skip the server health check. Please use yarn start to verify the server manually'
    })
        .positional('--use-max-memory-heap', {
        describe: `Use max memory heap for running the pack command, equal to "set NODE_OPTIONS=--max_old_space_size=4096"`
    })
        .positional('--skip-validation', {
        describe: `Skips package validation during the pack process`
    })
        .example('yarn msdyn365 pack myPackage', 'Optionally names the package as myPackage')
        .usage('usage: yarn msdyn365 pack [packageName]');
}, (args) => {
    checkExtraArguments(args._);
    pack_1.default(args.packageName, args['skip-health-check'], args['use-max-memory-heap'], args['skip-validation'])
        .then(() => {
        utilities_internal_1.trace.info('pack command completed');
    })
        .catch(err => {
        utilities_internal_1.trace.info(`pack command failed with ${err}`);
        process.exit(1);
    });
})
