...
const renameCopiedModuleDirectories = (modulesBasePath, oldModuleName, newModuleName) => {
    ...
    const sortedEditablePath = editablePathNames.sort().reverse();
    // Rename directories first
    sortedEditablePath.forEach((pathName) => {
        // Limiting rename to only top level directory
        if (path.basename(pathName).indexOf(oldModuleName) !== -1) {
            const oldName = pathName;
            const oldNameRegex = new RegExp(`${oldModuleName}`, 'g');
            const newNameBase = path.basename(oldName).replace(oldNameRegex, newModuleName);
            const newName = path.join(path.dirname(oldName), newNameBase);
            try {
    ...
    paths.forEach((pathName) => {
        ...
        if (path.basename(pathName.path).indexOf(oldModuleName) !== -1) {
            ...
            if (filePath.includes('.definition.json')) {
                updateContentInDefinitionFile(filePath, oldModuleName, newModuleName, modulePath, pathName.path);
            }
                ...
                renamePlaceholdersInTsFiles(oldModuleName, newModuleName, pathName, modulePath);
            }
            // Rename files
            try {
        ...
        else if (pathName.path.includes('.ts')) {
            renamePlaceholdersInTsFiles(oldModuleName, newModuleName, pathName, modulePath);
        }
        firstImportPath = false;
    });
};
/**
 * Get excluded namespace regex.
 * this will exclude moduleName convertion if the import path is poiting to module reference -
 * import ... from <supported_namespace>/<source_module_name>
 */
const excludeNamesapceRegex = () => {
    const supportedNameSpace = utilities_internal_1.getSupportedNamespaces();
    // positive lookbehind to check if the import path has from.
    let regexString = '(?<=from .*)';
    for (const nameSpace of supportedNameSpace) {
        // negative lookbehind to check if the import path doest not include the supported namespaces.
        regexString = regexString.concat(`(?<!${nameSpace}\/.*)`);
    }
    return regexString;
};
/**
 * optimize the resolved absolute path to module's reference
 * c:/fabrikam/node_modules/@msdyn365_modules/<module>/src/<files> ==> @msdyn365_modules/<module>
 * this is based on a assumption that source modules export evertying in its index.ts file.
 */
const resolvedPathOptimizely = (resolvedPath) => {
    let resolvedPathOpt = resolvedPath;
    const optimizedImportPathRegex = new RegExp(`.*node_modules\/(.*)\/src.*`);
    const optimizedImportPathArray = optimizedImportPathRegex.exec(resolvedPath);
    if (optimizedImportPathArray && optimizedImportPathArray.length > 1) {
        resolvedPathOpt = optimizedImportPathArray[1];
    }
    return resolvedPathOpt;
};
// import module_name from '@namespace/module' => import {module_name} from '@namespace/module';
const resolveDefaultImportModule = (importModule) => {
    importModule = importModule.trim();
    if (!importModule.startsWith('{')) {
        const importModuleWithoutBrakets = importModule.replace(/\{/g, '').replace(/\}/g, '');
        const tmpImportMouels = `{ ${importModuleWithoutBrakets} }`;
        importModule = tmpImportMouels;
    }
    return importModule;
};
/**
 * Get resole import path from the source module
 */
const resolveSourceModuleImportPath = (matchedImportPath, oldModuleName, newModuleDirRaw, newFilePathRaw) => {
    let rulesDisable = '';
    if (!firstImportPath) {
        rulesDisable = `/* tslint:disable: no-duplicate-imports */\n`;
        firstImportPath = true;
    }
    const retrieveRegex = new RegExp(`import(.*)from '(.*)';`, 'g');
    const importPathArray = retrieveRegex.exec(matchedImportPath);
    let importPath = '';
    let importNames = '';
    if (importPathArray && importPathArray.length > 2) {
        importPath = importPathArray[2].trim();
        importNames = importPathArray[1].trim();
    }
    // only resolve those packages import outside of module dir.
    if (importPath && importPath.startsWith('../')) {
        const newModuleDir = newModuleDirRaw.replace(/\\/g, '/');
        const newFilePath = newFilePathRaw.replace(/\\/g, '/');
        const resolvedPathOfFile = path
            .resolve(copyableModulesFileMap[oldModuleName], path.relative(newModuleDir, newFilePath))
            .replace(/\\/g, '/');
        let resolvedPath = path.resolve(path.dirname(resolvedPathOfFile), importPath).replace(/\\/g, '/');
        const localResolvedPath = path.resolve(path.dirname(newFilePath), importPath).replace(/\\/g, '/');
        // only resolve resolve the import path to module's reference if the resolved path is outside of the module directory.
        if (resolvedPath.includes('node_module') && !localResolvedPath.includes(newModuleDir)) {
            // assume that all imports are exported from module's index file,
            // only keep module's path in import path.
            resolvedPath = resolvedPathOptimizely(resolvedPath);
            importNames = resolveDefaultImportModule(importNames);
            return `${rulesDisable}import ${importNames} from '${resolvedPath}';`;
        }
    }
    return `${rulesDisable}${matchedImportPath}`;
};
/**
 * Rename placeholders in TS files
 */
const renamePlaceholdersInTsFiles = (oldModuleName, newModuleName, pathName, newModuleDir) => {
    ...
    // get regex to exclude suported namespace
    const excludeNamespaceRegex = excludeNamesapceRegex();
    const viewImportRegex = new RegExp(`${excludeNamespaceRegex}${oldModuleName}`, 'g');
    const importPathRegex = new RegExp(`import.*from.*`, 'g');
    ...
    const options = {
        files: pathName.path,
        from: [importPathRegex, autoGenConfigClassName, autoGenPropsClassName, dataClassName, autoGenResourcesClassName, viewImportRegex],
        to: [
            (matchedImportPath) => {
                return resolveSourceModuleImportPath(matchedImportPath, oldModuleName, newModuleDir, pathName.path);
            },
...
const resolveDataActionDefinitionPath = (filePath) => {
    let resolvedPathOpt = filePath;
    const optimizedImportPathRegex = new RegExp(`.*node_modules\/(.*\/src.*)`);
    const optimizedImportPathArray = optimizedImportPathRegex.exec(filePath);
    if (optimizedImportPathArray && optimizedImportPathArray.length > 1) {
        resolvedPathOpt = optimizedImportPathArray[1].replace('src', 'dist/lib');
    }
    return resolvedPathOpt;
};
/**
 ...
 */
const updateContentInDefinitionFile = (filePath, oldModuleName, newModuleName, newModuleDirRaw, newFilePathRaw) => {
    ...
    if (defN) {
        ...
        if (defN.dataActions) {
            Object.keys(defN.dataActions).forEach(dataAction => {
                let origionalPath = defN.dataActions[dataAction].path;
                if (origionalPath && origionalPath.startsWith('../')) {
                    const newModuleDir = newModuleDirRaw.replace(/\\/g, '/');
                    const newFilePath = newFilePathRaw.replace(/\\/g, '/');
                    const resolvedPathOfFile = path
                        .resolve(copyableModulesFileMap[oldModuleName], path.relative(newModuleDir, newFilePath))
                        .replace(/\\/g, '/');
                    const resolvedPath = path.resolve(path.dirname(resolvedPathOfFile), origionalPath).replace(/\\/g, '/');
                    const localResolvedPath = path.resolve(path.dirname(newFilePath), origionalPath).replace(/\\/g, '/');
                    // only resolve the import path to module's reference if the resolved path is outside of the module directory.
                    if (resolvedPath.includes('node_module') && !localResolvedPath.includes(newModuleDir)) {
                        origionalPath = resolveDataActionDefinitionPath(resolvedPath);
                    }
                    defN.dataActions[dataAction].path = origionalPath;
                }
            });
        }
