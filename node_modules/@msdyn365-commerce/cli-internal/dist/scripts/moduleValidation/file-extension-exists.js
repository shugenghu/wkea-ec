 ...
 */
const validatePreviewFiles = (modulePath, status, validator, type = 'module') => {
    const previewFilesPattern = type === 'theme'
        ? path.join(modulePath, "previews/modules" /* THEME_PREVIEWS_MODULE_DIR */, '*', '*')
        : path.join(modulePath, "previews" /* MODULE_PREVIEW_DIR */, '*');
    const previewFiles = utilities_internal_1.safeGetAllFilesPathSync(previewFilesPattern);
    previewFiles.forEach(filename => {
        if (!filename.endsWith(".preview.json" /* PREVIEW_FILE */)) {
            utilities_internal_1.trace.error(`Unsupported preview file extension ${filename} found in preview folder`);
...
const validateDefinitionExtensions = (modulePath, status, validator) => {
    ...
    if (fs.existsSync(definitionExtensionsPath)) {
        ...
        definitionExtensions.forEach((name) => {
            ...
            const extValidationResult = utilities_internal_1.runSchemaValidationWithNestedError(validator, utilities_internal_1.safeReadJsonSync(path.resolve(definitionExtensionsPath, name)), 
        ...
        defaultStylePreset.forEach((name) => {
            ...
            if (stylePresetFileName === themeName) {
                if (definitionThemeName === stylePresetFileName) {
                    ...
                    const validationResult = utilities_internal_1.runSchemaValidationWithNestedError(validator, utilities_internal_1.safeReadJsonSync(path.resolve(styleDefinitionsFilePath, `${themeName}.definition.scss.json`)), 
                    // tslint:disable-next-line:no-any
...
const validateStylePresetInstance = (status, validator, styleDefinitionsFilePath, stylePresetDefinitionFile, stylePresetInstanceFiles) => {
    ...
    for (let i = 0; i < stylePresetInstanceFiles.length; i++) {
        ...
        if (readJSON.name === stylePresetInstanceFileName) {
            const validationResult = utilities_internal_1.runSchemaValidationWithNestedError(validator, utilities_internal_1.safeReadJsonSync(path.resolve(styleDefinitionsFilePath, `${readJSON.name}.scss.json`)), stylePresetSchema);
            if (validationResult.errors.length > 0) {
...
module.exports = (options) => {
    ...
    if (moduleDefinitionFile['$type'] === 'themeModule') {
        ...
        validatePreviewFiles(modulePath, status, validator, 'theme');
    }
    // tslint:disable-next-line: no-string-literal we know this property is required on definition files
    if (moduleDefinitionFile['$type'] === 'contentModule') {
        validatePreviewFiles(modulePath, status, validator, 'module');
    }
    ...
    const validationResult = utilities_internal_1.runSchemaValidationWithNestedError(validator, moduleDefinitionFile, 
    // tslint:disable-next-line:no-any
    utilities_internal_1.safeReadJsonSync(path.resolve(__dirname, '../..', 'definition-schema.json')));
    if (validationResult.errors.length > 0) {
