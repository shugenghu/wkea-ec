...
// we need to import validateStation from dist/ because it dynamicly import js modules.
// tslint:disable:no-require-imports
const validateStation = require('../dist/scripts/validation-station');
...
const defaultDataDirForTheme = path.resolve(srcDir, 'themes', themeName, 'previews', 'modules');
const defaultDataDirForModule = path.resolve(srcDir, 'modules', moduleName, 'previews');
...
describe('test add-module', () => {
    ...
    it('runs add-module and validation passes', () => __awaiter(void 0, void 0, void 0, function* () {
        add_module_1.default(moduleName);
        // create default data to theme.
        fs.mkdirSync(defaultDataDirForModule, { recursive: true });
        fs.appendFileSync(path.resolve(defaultDataDirForModule, 'data.preview.json'), '');
        const validationResult = yield validateStation.validatePackage(process.cwd());
        if (!validationResult.success) {
            throw new Error('add-module creates a module that does not pass validation');
        }
        // create wrong default data to theme.
        fs.appendFileSync(path.resolve(defaultDataDirForModule, 'wrong.json'), '');
        const worngValidationResult = yield validateStation.validatePackage(process.cwd());
        expect(worngValidationResult.success).toEqual(false);
    }));
});
...
describe('test pack command', () => {
    ...
    it('check if pack command packs with provided name', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        yield pack_1.default('testPackage');
...
describe('test add-theme', () => {
    ...
    it('runs add-theme and validation passes', () => __awaiter(void 0, void 0, void 0, function* () {
        add_theme_1.default(themeName);
        const defaultDataFolder = path.resolve(defaultDataDirForTheme, 'testModule');
        // create default data to theme.
        fs.mkdirSync(defaultDataFolder, { recursive: true });
        fs.appendFileSync(path.resolve(defaultDataFolder, 'data.preview.json'), '');
        const validationResult = yield validateStation.validatePackage(process.cwd());
        if (!validationResult.success) {
            throw new Error('add-theme creates a theme that does not pass validation');
        }
        // create wrong default data to theme.
        fs.appendFileSync(path.resolve(defaultDataFolder, 'wrong.json'), '');
        const worngValidationResult = yield validateStation.validatePackage(process.cwd());
        expect(worngValidationResult.success).toEqual(false);
    }));
});
