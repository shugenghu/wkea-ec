...
const { mergeExtendedDefinitionFile, EXTENDED_DEFINITION_FILE_EXTENSION, shouldExtendDependentConfigProperty } = require('./utils/helpers');
...
let dependentPropertiesMergeCount = 0 ;
...
class ModuleDefinitionGenerator {
    ...
    normalizeType(fieldObject, fieldName) {
        ...
        switch (type) {
            ...
            case 'object':
                ...
                return fieldName;
            case 'array':
                if (!fieldObject.items) {
                    trace.error(`ERROR: ${fieldName} was defined as an array type but does not include required 'items' object.`);
                    return undefined;
                }
            ...
            case 'customDefinitionType':
                return fieldObject.interfaceName;
            default:
                return type;
        }
    }
    ...
    markLocalRefObjects(configProperties) {
        if (configProperties) {
            for (const field in configProperties) {
                ...
                } else if (fieldObject.type === 'array' && fieldObject.items && fieldObject.items['$ref'] && fieldObject.items['$ref'].indexOf('#') === 0) {
    ...
    /**
     * Function to check and merge config propeties from dependentSchemas to Config in module definition to generate props
     * @param {*} configs 
     * @param {*} dependenciesProperty 
     */
    mergeConfigPropertiesFromDependentSchemas(configs, dependenciesProperty, moduleName) {
        if(dependenciesProperty) {
            const entries = dependenciesProperty && Object.keys(dependenciesProperty);
            if (entries && entries.length > 0) {
                shouldExtendDependentConfigProperty(entries, dependenciesProperty, configs, 'config', false, `${moduleName}.definition.json`);
            }
        }
    }
    ...
    getConfigInterfaces(configs = this.json.config, interfaceTitle = this.configInterfaceTitle) {
        ...
        // Only merge the config properties from DependenctSchemas once, to handle inline objects
        if(this.json.dependentSchemas && dependentPropertiesMergeCount < 1) {
            this.mergeConfigPropertiesFromDependentSchemas(configs, this.json.dependentSchemas, this.json.name);
            dependentPropertiesMergeCount++;
        }
        ...
        for (const field in configs) {
            ...
            // If an object within definitions references another definition property through $ref
            // add type and interface name here so that normalizeType function can return the appropriate TypeScript type
            if (fieldObject.$ref) {
                // Split the ref to find the property name
                const refParts = fieldObject.$ref.split('/');
                // Indicates definition is referencing another local definition object
                if (refParts[0] === '#') {
                    const defNObject = refParts[refParts.length - 1];
                    const interfaceName = this.definitionObjectsToInterfaceName[defNObject];
                    if (interfaceName) {
                        fieldObject.type = 'customDefinitionType';
                        fieldObject.interfaceName = interfaceName;
                    }
                } else {
                    // If definition object is referencing an external definition, display error message as we don't support that scenario
                    trace.error(
                        `Error processing definitions object in definitions.[${field}]. Accessing definitions from external sources within definitions is not supported.`
                    );
                    continue;
                }
