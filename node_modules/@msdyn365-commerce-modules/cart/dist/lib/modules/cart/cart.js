...
import { Alert, Button, getPayloadObject, getTelemetryAttributes, getTelemetryObject, TelemetryConstant, Waiting } from '@msdyn365-commerce-modules/utilities';
import { PromoCodeComponent, SaveCartToOrderTemplateComponent } from '@msdyn365-commerce/components';
...
let Cart = class Cart extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        const { data: { cart: { result: cart } }, context: { request: { user: { isAuthenticated, signInUrl } } }, resources } = this.props;
        ...
        const viewProps = {
            ...
            title: React.createElement("h1", { className: 'msc-cart__heading' }, resources.cartTitle),
            ...
            orderSummaryHeading: this.props.data.products && this.props.data.products.result && cart && cart.cart.CartLines && cart.cart.CartLines.length > 0 ?
                (React.createElement("h2", { className: 'msc-order-summary__heading' }, resources.orderSummaryTitle)) : null,
            orderSummaryLineitems: this._getOrderSummaryLineItems(),
            checkoutAsSignInUserButton: (!isCartEmpty) ?
                (React.createElement(Button, Object.assign({ disabled: hasErrors, className: 'msc-cart__btn-checkout', href: checkoutURL, title: resources.checkoutButtonTitle }, checkoutBySignInAttributes), resources.checkoutButtonTitle)) : null,
            checkoutAsGuestButton: (!isCartEmpty && !isAuthenticated) ?
                (React.createElement(Button, Object.assign({ disabled: hasErrors, className: 'msc-cart__btn-guestcheckout', href: guestCheckoutUrl, title: resources.guestCheckoutButtonTitle }, checkoutByGuestAttributes), resources.guestCheckoutButtonTitle)) : null,
            backToShoppingButton: React.createElement(Button, Object.assign({ className: 'msc-cart__btn-backtoshopping' }, backtoShoppingAttributes, { href: backToShoppingUrl, title: resources.backToShoppingButtonTitle }), resources.backToShoppingButtonTitle),
            createTemplateFromCartButton: (this._displayAddCartToTemplate()) ?
                (React.createElement(SaveCartToOrderTemplateComponent, { addToOrderTemplateDialogResources: this._generateAddToOrderTemplateDialogResources(resources), createOrderTemplateDialogResources: this._generateOrderTemplateNameDialogResources(resources), linesAddedToOrderTemplateDialogResources: this._generateLinesAddedToOrderTemplateDialogResources(resources), createOrderTemplateFromCartButton: resources.createOrderTemplateFromCartButton, context: this.props.context, id: this.props.id, typeName: this.props.typeName, data: {
            ...
            waitingComponent: React.createElement(Waiting, { className: 'msc-waiting-circular msc-waiting-lg' }),
    ...
    _getOrderSummaryErrors(cartLines) {
        const allErrors = cartLines && cartLines.map(cartline => {
            ...
            if (!isServiceItem && cartline.error) {
                ...
                return (React.createElement("div", { className: 'msc-alert__line' }, fullError));
            }
            return null;
        }).filter((line) => !!line) || [];
        return {
            ...
            header: (React.createElement("div", { className: 'msc-alert__header', "aria-hidden": 'true' },
                React.createElement("span", { className: 'msi-exclamation-triangle' }),
                React.createElement("span", null, this.props.resources.outOfStockErrorMessageHeader))),
            errors: allErrors
        };
    }
    _getOrderSummaryLineItems() {
        const { data: { cart: { result: cart } }, telemetry, resources } = this.props;
    ...
    _getCartLines() {
        const { data: { cart: { result: cart }, wishlists: { result: wishlists }, orderTemplates: { result: orderTemplates }, customerInformation: { result: customerInformation } }, context: { request: { channel: { PickupDeliveryModeCode } = { PickupDeliveryModeCode: undefined } } }, config: config, telemetry, resources } = this.props;
