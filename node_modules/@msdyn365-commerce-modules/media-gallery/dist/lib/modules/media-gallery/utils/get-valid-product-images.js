...
export async function getValidProductImages(productId, channelId, actionContext, imageSettings) {
    const actionInput = new MediaLocationsForSelectedVariantInput(productId, channelId);
...
export async function validateProductImages(mediaLocations, actionContext, imageSettings) {
    return Promise.all(mediaLocations.map((mediaLocation) => validateMediaLocaionAsync(mediaLocation, imageSettings)))
        .then(pairs => {
        return pairs.filter(pair => pair[1]).map(pair => pair[0]);
    }).catch(error => {
...
async function validateMediaLocaionAsync(mediaLocation, imageSettings) {
    const imageData = {
...
//# sourceMappingURL=get-valid-product-images.js.map